Vue.component('Update', {
	template: `
		<el-dialog title="修改" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
			<el-form :size="size" ref="form" :model="form" :rules="rules" :label-width=" ismobile()?'90px':'16%'">
				...
				<el-row >
					<el-col :span="24">
						<el-form-item label="单图上传带裁剪" prop="pic">
							<Upload v-if="show" size="small"      file_type="image" cropsize="300,300" :image.sync="form.pic"></Upload>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row >
					<el-col :span="24">
						<el-form-item label="单图2" prop="pic_2">
							<Upload v-if="show" size="small"  upload_type="2"    file_type="image"  :image.sync="form.pic_2"></Upload>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row >
	...
	methods: {
		open(){
			this.form = this.info
			this.setDefaultVal('pics')
			this.setDefaultVal('files')
		},
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Uploadfile/update',this.form).then(res => {
